...
func NewRawSocketEngine(protocol int) (*RawSocketEngine, error) {
	fmt.Println("Creating raw socket for protocol:", protocol) // 日志：创建 socket
	sendFD, err := syscall.Socket(syscall.AF_INET, syscall.SOCK_RAW, syscall.IPPROTO_RAW)
	if err != nil {
		return nil, fmt.Errorf("create send socket: %v", err)
	}
	if runtime.GOOS == "darwin" {
		fmt.Println("Setting IP_HDRINCL on macOS") // 日志：macOS 设置
	...
	fmt.Println("Raw socket created: sendFD =", sendFD, "recvFD =", recvFD)
	return &RawSocketEngine{sendFD: sendFD, recvFD: recvFD}, nil
}

func (e *RawSocketEngine) SendPacket(target net.IP, packet []byte) error {
	addr := syscall.SockaddrInet4{Addr: [4]byte(target.To4())}
	fmt.Printf("Sending packet to %s: %x\n", target, packet) // 日志：发送包
	err := syscall.Sendto(e.sendFD, packet, 0, &addr)
	if err != nil {
		fmt.Println("SendPacket error:", err) // 日志：发送错误
	}
	return err
}

func (e *RawSocketEngine) ReceivePackets(results chan<- []byte) {
	f := os.NewFile(uintptr(e.recvFD), fmt.Sprintf("fd %d", e.recvFD))
	buf := make([]byte, 1500)
	for {
		n, err := f.Read(buf)
		if err != nil {
...
func BuildIPHeader(srcIP, dstIP net.IP, protocol layers.IPProtocol, totalLen uint16) []byte {
	...
	copy(ipHeader[16:20], dstIP.To4())
	if runtime.GOOS == "darwin" {
		binary.LittleEndian.PutUint16(ipHeader[2:4], totalLen)
		binary.BigEndian.PutUint16(ipHeader[4:6], uint16(rand.Intn(65535)))
		binary.BigEndian.PutUint16(ipHeader[10:12], calculateIPChecksum(ipHeader))
	}
	fmt.Printf("Built IP header: %x\n", ipHeader) // 日志：IP 头
	return ipHeader
}
